...
const checks = [
  ...
  { id: 'authoritative-action-preflight', ok: has('app/src/hooks/useContractWrite.js', /simulateContract/) && has('app/src/components/actions/SubmitConfirmation.jsx', /Authoritative chain preflight/) },
  { id: 'optimistic-action-feedback', ok: has('app/src/components/actions/ActionPanel.jsx', /optimisticSubmitted/) && has('app/src/components/actions/ReceiptDrawer.jsx', /Optimistic intent/) },
  ...
  { id: 'browser-sponsored-actions', ok: exists('app/src/hooks/useSponsoredSession.js') && exists('app/src/components/actions/SponsoredSessionControl.jsx') && has('app/e2e/visual.spec.js', /sponsored turn without a second wallet transaction/) },
  { id: 'local-sponsored-stack', ok: has('scripts/run-local-stack.mjs', /sponsor-relay-server\.mjs/) && has('scripts/run-local-stack.mjs', /VITE_SPONSOR_RELAY_URL/) },
