...
describe('final art expansion catalog', () => {
  it('covers every authored hero location with unique optimized backplates', () => {
    expect(FINAL_ART_EXPANSION_VERSION).toBe('2026-09-13.1');
    expect(Object.keys(GUEST_LOCATION_ART)).toHaveLength(8);
    expect(new Set(Object.values(GUEST_LOCATION_ART)).size).toBe(8);
  ...
  it('connects encounter, crew ability, and future relic art to gameplay ids', () => {
    ...
    expect(FUTURE_RELIC_SIGNALS.map(({ id }) => id)).toEqual(['echo-compass', 'stormglass-seed']);
  });
});
