...
Then open `http://localhost:5502/simulator`. The runner writes reports to `reports/simulator/latest-report.json` and `app/public/simulator/latest-report.json`, scores them against `simulator.tuning.json`, keeps synthetic-player policy in `simulator.agent-policies.json` separate from the immutable `simulator.evaluation.json` rubric, runs the Fun Debugger analysis, applies supported Scenario Setup Forge starting conditions, evaluates Gameplay Oracle verdicts, and feeds the downstream Memory, Time Machine, Lab, Feeling, Tutor, Evidence Bridge, and Fun systems. `npm run gameplay:refresh` rebuilds that dependency graph. `npm run gameplay:refresh:exact` starts and stops its own hidden local chain, selects an available RPC port, resumes valid scenario checkpoints, reruns the canonical same-engine matrix, and then rebuilds every derived report. `npm run gameplay:regression` runs the smaller boundary-fixture pack.

See [docs/gameplay-improvement-system.md](docs/gameplay-improvement-system.md) for the complete evidence loop, [docs/gameplay-simulator.md](docs/gameplay-simulator.md) for strategies and engine-path details, [docs/scenario-setup-forge.md](docs/scenario-setup-forge.md) for scenario setup support, [docs/scenario-autopilot.md](docs/scenario-autopilot.md) for the orchestration loop, [docs/gameplay-oracle.md](docs/gameplay-oracle.md) for Oracle scoring and daily tuning workflows, [docs/playable-design-memory.md](docs/playable-design-memory.md) for memory queries and report outputs, [docs/scenario-time-machine.md](docs/scenario-time-machine.md) for scenario trend reports, [docs/scenario-lab-notebook.md](docs/scenario-lab-notebook.md) for design journal entries and decisions, [docs/player-feeling-black-box.md](docs/player-feeling-black-box.md) for felt-control arcs, [docs/scenario-self-driving-tutor.md](docs/scenario-self-driving-tutor.md) for ordered gameplay lessons, and [docs/scenario-evidence-bridge.md](docs/scenario-evidence-bridge.md) for public route readiness.

Public growth routes start at `http://localhost:5502/play`, with `/challenge`, `/scenarios`, `/progress`, `/devlog`, and `/create-scenario` covering the shareable run loop. See [docs/growth-loop.md](docs/growth-loop.md) for metrics, routes, and release checks.

Fun-first public run tuning is documented in [docs/fun-loop.md](docs/fun-loop.md). It adds action previews, barks, event cards, artifacts, roles, badges, run titles, and `npm run fun:report` quality gates.

Scenario Evidence Bridge is documented in [docs/scenario-evidence-bridge.md](docs/scenario-evidence-bridge.md). It writes ignored `reports/bridge/` and `app/public/bridge/` JSON, then `/play`, `/challenge`, `/scenarios`, `/progress`, `/devlog`, and `/create-scenario` use readiness to explain what is featured and what still needs evidence.
